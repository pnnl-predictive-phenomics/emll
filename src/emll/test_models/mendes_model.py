...
def reversible_hill(substrate, product, Modifier, Keq, Vf, Shalve, Phalve, h, Mhalve, alpha):
    return (
        ...
        * substrate
        / Shalve
        * (1 - product / (substrate * Keq))
        * (substrate / Shalve + product / Phalve) ** (h - 1)
        / (
            (1 + (Modifier / Mhalve) ** h) / (1 + alpha * (Modifier / Mhalve) ** h)
            + (substrate / Shalve + product / Phalve) ** h
        )
    )


def ordered_bi_bi(
    substratea, substrateb, productp, productq, Keq, Vf, Vr, Kma, Kmb, Kmp, Kmq, Kia, Kib, Kip
):
    return (
        Vf
        * (substratea * substrateb - productp * productq / Keq)
        / (
            substratea * substrateb * (1 + productp / Kip)
            + Kma * substrateb
            + Kmb * (substratea + Kia)
            + Vf
            / (Vr * Keq)
            * (
                Kmq * productp * (1 + substratea / Kia)
                + productq
                * (Kmp * (1 + Kma * substrateb / (Kia * Kmb)) + productp * (1 + substrateb / Kib))
            )
        )
    )
...
x_ss = np.array([1.0, 0.996, 0.164, 0.0428, 0.101, 0.0726, 0.202, 0.0202, 0.0789, 0.1, 0.2])
