...
def create_noisy_observations_of_computed_values(name:str, computed_tensor:pt.tensor, data:pd.DataFrame, estimated_stdev:pd.DataFrame)->dict[tuple[int,int],pt.tensor]:  # noqa: D417
    """Create noisy observations of computed values.

    Parameters
    ----------
        name (str): The name of the computed tensor.
        computed_tensor (pt.tensor): The computed tensor used for mu (mean).
        data (pd.DataFrame): The data used for observations.
        estimated_stdev (pd.DataFrame): The estimated standard deviation.

    Returns
    -------
        dict[tuple[int,int],pt.tensor]: A dictionary containing the noisy observations of computed values.
